...
import type { CreateMoneyItemPayLoad } from '@/types/money';

type CreateMoneyItemResult =
  | { success: true }
...
function useCreateMoneyItem() {
  const [isLoading, setIsLoading] = useState(false);
  const queryClient = useQueryClient();

  const handleCreateMoneyItem = async (
    draft: MoneyDraft,
  ): Promise<CreateMoneyItemResult> => {
    ...
    try {
      await createMoneyItem(careGroupId, formatForPayload(draft));
  ...
  return { isLoading, handleCreateMoneyItem };
}

export default useCreateMoneyItem;
