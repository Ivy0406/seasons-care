...
import { getTemperatures } from '@/api/endpoints/health';
import useCurrentGroupId from '@/hooks/useCurrentGroupID';
import type { GetTemperaturesResponse, TemperatureData } from '@/types/health';

import healthKeys from '../queryKeys';

import type { AxiosResponse } from 'axios';

const defaultData: TemperatureData = { date: '--', time: '--', value: '--' };

function transformToLatestToday(
  res: AxiosResponse<GetTemperaturesResponse>,
): TemperatureData {
  const now = new Date();
...
function useGetTemperature() {
  const { currentGroupId } = useCurrentGroupId();

  return useQuery({
    queryKey: currentGroupId
      ? healthKeys.temperature(currentGroupId)
      : healthKeys.all,
    queryFn: () => getTemperatures(currentGroupId ?? ''),
