...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        'fixed inset-0 z-50 bg-black/10 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:bg-black/25',
        className,
      )}
      {...props}
    />
      ...
      >
        ...
        {showCloseButton && (
          <SheetPrimitive.Close
            ...
            render={
              <CircleButtonPrimary
                className="absolute top-16.75 right-6 border-neutral-100"
                size="md"
              >
                <X />
              </CircleButtonPrimary>
